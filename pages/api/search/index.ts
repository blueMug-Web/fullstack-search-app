...
export default function handler(
  ...
  //Same as 'const searchTerm = req.body.searchTerm'
  const {
    body: { searchTerm },
  } = req;

  if (req.method === 'POST' && searchTerm && searchTerm.length > 0) {
    // Creates a regex search pattern for a case insensitive match from the user's search term (searchTerm is user's entered search, 'i' is for "case insensitive" as we don't care if searchTerm is all caps or not)
